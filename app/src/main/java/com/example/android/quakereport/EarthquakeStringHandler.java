...
public class EarthquakeStringHandler {
    ...
    public static List<Earthquake> getList(String s) {

        List<Earthquake> earthquakes = new ArrayList<>();

        try {
            JSONObject originalJsonObj = new JSONObject(s);
            JSONArray featuresJsonArray = originalJsonObj.getJSONArray("features");

            for (int i = 0; i < featuresJsonArray.length(); i++) {

                Earthquake e = new Earthquake();

                JSONObject eachJsonObject = (JSONObject) featuresJsonArray.get(i);
                e.setmMagnitude(eachJsonObject.getJSONObject("properties").getDouble("mag"));
                e.setmLocationDetails(eachJsonObject.getJSONObject("properties").getString("place"));
                e.setmTimeMillis(eachJsonObject.getJSONObject("properties").getLong("time"));
                e.setmUrl(eachJsonObject.getJSONObject("properties").getString("url"));
...
//
//        List<Earthquake> earthquakes = Arrays.asList(objectMapper.readValue(dataString, Earthquake[].class));
//
